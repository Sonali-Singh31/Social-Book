...
def index(request):
    ...
    final_suggestion = [x for x in list(new_suggestion_list) if(x not in list(current_user)) ]
    ...
    return render(request,'index.html',{'user_profile':user_profile,'posts':feed_list,'suggestions_username_profile_list':suggestions_username_profile_list[:4],'length':length})
...
def profile(request,pk):
    ...
    user = pk
    if FollowersCount.objects.filter(followers=follower,user=user):
...
def settings(request):
    try:
        ...
        # Optionally, create a Profile object if not found
...
def signup(request):
    if request.method == 'POST':
        ...
        if not username or not email or not password or not password2:
            messages.info(request, 'Provide all credentials')
